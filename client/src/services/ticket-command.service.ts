...
function waitForResult(commandId: string): Promise<CommandStatus> {
  ...
  const pollStatus = async (resolve: any, reject: any) => {
    ...
    const res = await fetch(buildUrl(`/tickets/commands/${commandId}`));
    if (res.status == 200) {
...
export async function sendTicketCommand(url: string, body: any, options? : SendCommandOptions): Promise<CommandStatus> {
