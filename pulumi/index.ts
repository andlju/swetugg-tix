...
export = async () => {
  ...
  const resourceGroup = new azure.resources.ResourceGroup(`${baseName}-group`, {
  ...
  const serviceBusAuthRule = new azure.servicebus.NamespaceAuthorizationRule(
    `${baseName}-bus-rule`,
    {
      namespaceName: serviceBusNamespace.name,
      resourceGroupName: resourceGroup.name,
  ...
  // Redis Cache
  //
  const redisCache = new azure.cache.Redis(
    "cmds",
    {
      resourceGroupName: resourceGroup.name,
      sku: {
  ...
  const redisCacheKeys = pulumi
    .all([resourceGroup.name, redisCache.name])
...
*/
  ...
  const eventHubAuthRule = new azure.eventhub.NamespaceAuthorizationRule(
    "tixhub-authrule",
    {
      resourceGroupName: resourceGroup.name,
  ...
  const activityViewsConsumerGroup = new azure.eventhub.ConsumerGroup(
    "activityviews",
    {
      resourceGroupName: resourceGroup.name,
      namespaceName: eventHubNamespace.name,
  ...
  const activityProcessConsumerGroup = new azure.eventhub.ConsumerGroup(
    "activityproc",
    {
      resourceGroupName: resourceGroup.name,
      namespaceName: eventHubNamespace.name,
  ...
  const orderViewsConsumerGroup = new azure.eventhub.ConsumerGroup(
    "orderviews",
    {
      resourceGroupName: resourceGroup.name,
      namespaceName: eventHubNamespace.name,
  ...
  const orderProcessConsumerGroup = new azure.eventhub.ConsumerGroup(
    "orderproc",
    {
      resourceGroupName: resourceGroup.name,
      namespaceName: eventHubNamespace.name,
  ...
  // Collect settings
  const activityEventStoreConnection = pulumi
    .all([
      sqlServer.name,
      activityEventStoreDatabase.name,
      sqlAdminPassword.result,
    ])
    .apply(
      ([server, db, pwd]) =>
  ...
  const orderEventStoreConnection = pulumi
    .all([
      sqlServer.name,
      orderEventStoreDatabase.name,
      sqlAdminPassword.result,
    ])
    .apply(
      ([server, db, pwd]) =>
  ...
  const processEventStoreConnection = pulumi
    .all([
      sqlServer.name,
      processEventStoreDatabase.name,
      sqlAdminPassword.result,
    ])
    .apply(
      ([server, db, pwd]) =>
  ...
  const tixViewsConnection = pulumi
    .all([sqlServer.name, tixViewsDatabase.name, sqlAdminPassword.result])
    .apply(
      ([server, db, pwd]) =>
        `Server=tcp:${server}.database.windows.net,1433;Initial Catalog=${db};User ID=${sqlAdminUser};password=${pwd};Persist Security Info=False;Encrypt=True;MultipleActiveResultSets=False;Connection Timeout=30;`
    );

  const primaryServiceBusKey = await pulumi
    .all([
      resourceGroup.name,
      serviceBusNamespace.name,
      serviceBusAuthRule.name,
    ])
    .apply(async ([resourceGroupName, namespaceName, authRuleName]) => {
  ...
  const storageAccountConnection = getConnectionString(resourceGroup.name, storageAccount.name);

  const eventHubConnectionString = await pulumi
    .all([resourceGroup.name, eventHubNamespace.name, eventHubAuthRule.name])
    .apply(
      async ([resourceGroupName, namespaceName, authorizationRuleName]) => {
  ...
  if (shouldDeploy) {
    ...
    const frontendResourceGroup = new azure.resources.ResourceGroup(
      `${baseName}-fe-group`,
      { location: mainLocation }
    );
      ...
      siteConfig: {
        ...
        appSettings: [
          ...
          { name: "WEBSITE_NODE_DEFAULT_VERSION", value: "12.18.0" },
          {
            ...
            value: pulumi.interpolate`https://${apiHostname}/api`,
          },
          {
    ...
    const backOfficeApp = new azure.web.WebApp(`${baseName}-backoffice`, {
      ...
      siteConfig: {
        ...
        appSettings: [
          ...
          {
            name: "NEXT_PUBLIC_FRONTPAGE_ROOT",
            value: pulumi.interpolate`https://${frontpageApp.defaultHostName}`,
          },
          {
            name: "NEXT_PUBLIC_APPINSIGHTS_INSTRUMENTATIONKEY",
            value: appInsights.instrumentationKey,
  ...
  const output = {
    ...
    storageConnectionString: storageAccountConnection,
