...
:: 3. KuduSync
CALL :ExecuteCmd "%KUDU_SYNC_CMD%" -v 50 -f ".\src\Swetugg.Tix.Activity.Jobs\project.json" -t "%DEPLOYMENT_TARGET%" -s "%WEBJOB_DEPLOYMENT_TARGET%" -n "%NEXT_MANIFEST_PATH%" -p "%PREVIOUS_MANIFEST_PATH%" -i ".git;.hg;.deployment;deploy.cmd"
IF !ERRORLEVEL! NEQ 0 goto error
