...
const loginSilentEpic: Epic<AuthAction, AuthAction, RootState> = (action$, state$) =>
  action$.pipe(
    ...
    mergeMap(([, state]) => {
      if (state.auth.user.current?.subject) {
        ...
        return msalService.ssoSilent({
          ...loginRequest,
          loginHint: state.auth.user.current?.subject,
        });
      }
      return of(login(InteractionKind.POPUP));
    }),
    catchError((err, caught) => {
      return caught.pipe(map(() => login(InteractionKind.POPUP)));
    }),
    map(() => loginCompleted())
  );

const loginPopupEpic: Epic<AuthAction, AuthAction, RootState> = (action$, state$) =>
  action$.pipe(
    filter(isOfType(AuthActionTypes.LOGIN)),
    ...
    catchError((err, caught) => {
      ...
      return caught.pipe(map(() => loginFailed('LoginFailed', errString)));
    }),
    map(() => loginCompleted())
  );

const loginRedirectEpic: Epic<AuthAction, AuthAction, RootState> = (action$, state$) =>
  action$.pipe(
    filter(isOfType(AuthActionTypes.LOGIN)),
...
const validateLoginEpic: Epic<AuthAction, AuthAction, RootState> = (action$) =>
  action$.pipe(
    filter(isOfType(AuthActionTypes.VALIDATE_LOGIN)),
    mergeMap((action) =>
      msalService.currentAccount$().pipe(
        mergeMap((account) => {
          if (account) {
...
const setUserEpic: Epic<AuthAction, AuthAction, RootState> = (action$, state$) =>
  withToken$(
    action$.pipe(
      filter(
        (action) =>
          ...
          isOfType(AuthActionTypes.UPDATE_USER_COMPLETE)(action)
      )
    ),
    state$
  ).pipe(
...
const requestUserUpdateEpic: Epic<AuthAction, AuthAction, RootState> = (action$, state$) =>
  action$.pipe(
...
const createUserEpic: Epic<AuthAction, AuthAction, RootState> = (action$, state$) =>
  action$.pipe(
    ...
    mergeMap(([action, token]) =>
      ajax.post(buildUrl(`/me`), action.payload.user, {
...
const updateUserEpic: Epic<AuthAction, AuthAction, RootState> = (action$, state$) =>
  action$.pipe(
    ...
    take(1),
    mergeMap(([action, token]) =>
      ajax.put(buildUrl(`/me`), action.payload.user, {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
      })
    ),
...
const currentAccessToken$ = (state$: StateObservable<RootState>) =>
  state$.pipe(
    map((state) => state.auth.accessToken),
    distinctUntilChanged()
  );

const currentUser$ = (state$: StateObservable<RootState>) =>
  state$.pipe(
...
export function withTokenAndUser$<TAction>(
  action$: Observable<TAction>,
  state$: StateObservable<RootState>
): Observable<[TAction, string | undefined, User | undefined]> {
...
export function withToken$<TAction>(
  action$: Observable<TAction>,
  state$: StateObservable<RootState>
...
export const authEpic = combineEpics(
  ...
  getScopesSilentEpic,
  getScopesPopupEpic,
  getScopesRedirectEpic,
  validateLoginEpic,
  inProgressEpic,
  setUserEpic,
  requestUserUpdateEpic,
  createUserEpic,
  updateUserEpic
);
